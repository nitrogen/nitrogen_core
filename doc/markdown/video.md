...
- `preload` ('auto' | 'metadata' | 'none') - If set to the atom `auto`, the
  browser will download the whole video when the page loads. If set to the atom
  `metadata`, the browser will download just the metadata of the video when the
  page loads. And if set to the atom `none`, the browser will not load the video
  at all when the page loads.

### See Also

- [base element](./element_base.md)
- [youtube element](./youtube.md)
