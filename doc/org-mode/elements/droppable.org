...
** Attributes
   ...
   + accept\_groups - (/list of atoms (defaults to 'all')/) :: The drag groups
     that will be accepted by this drop target

   + active\_class - (/atom or string (defaults to 'active')/) :: This CSS
     class will be applied to the element when the user drags a draggable
     element.

   + hover\_class - (/atom or string (defaults to 'hover')/) :: This CSS class
     will be applied to the element when the user hovers with a draggable
     element.

** Events

*** drop\_event(DragTag, DropTag)
