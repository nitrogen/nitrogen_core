...
render_element(#recaptcha{id=ID, class=Cl, delegate=Delegate,
               fail_body=FB, captcha_opts=COPts, btn_label=BtnLabel})->
    ...
    wf:wire(submit_recaptcha, #event{type=click, actions=[
        ...
        body=[
            ...
            #panel{body=[
                #panel{id=fail_msg,
                       ...
                       body=FB},
                #button{id=submit_recaptcha, style="float:right",
                        text=BtnLabel,
...
mk_post_request(Url, Params)->
    MkField = fun(K, V) ->
        lists:flatten([atom_to_list(K), "=", http_uri:encode(V)])
    end,
...
evaluate_captcha(CaptchaID, Challenge, Response, Fail) ->
    Url = verify_url(),
    ...
    case Re of
        {ok, {{_Version, 200, _Reason}, _Hdr, Body}} ->
            parse_response(CaptchaID, Body, Fail);
        {error, Reason}                              ->
            {error, Reason}
    end.
...
parse_response(CaptchaID, Body, Fail) ->
    case string:tokens(Body, "\n") of
        ["true", "success"] -> wf:remove(CaptchaID),
                               ok;
        ["false", Error]    -> wf:wire("Recaptcha.reload()"),
                               wf:replace(fail_msg,
                                  #panel{id=fail_msg,
...
event({eval_recaptcha, CaptchaID, Delegate, Fail}) ->
    ...
    Evaled = evaluate_captcha(CaptchaID, Challenge, Response, Fail),
    Module = wf:coalesce([Delegate, wf:page_module()]),
    Module:event({recaptcha, Evaled}).
