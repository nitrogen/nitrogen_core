...
something_that_changes_handler_state(Arg1, Arg2) ->
    {ok, _Value} = wf_handler:call(x_handler, something_that_changes_handler_state, [Arg1, Arg2]).


%% behavior functions that don't change handler state just return a value.
something_that_does_not_change_handler_state(Arg1) ->
    _Value = wf_handler:call_readonly(x_handler, something_that_does_not_change_handler_state, [Arg1]).
