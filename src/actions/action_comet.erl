...
accumulator_loop(Guardians, Actions, Waiting, TimerRef, WebsocketPid) ->
    receive
        {add_guardian, Pid} ->
            erlang:monitor(process, Pid),
            accumulator_loop([Pid|Guardians], Actions, Waiting, TimerRef, WebsocketPid);

        {'DOWN', _, process, Pid, _} ->
            accumulator_loop(Guardians -- [Pid], Actions, Waiting, TimerRef, WebsocketPid);

        {add_actions, NewActions} ->
            case is_remote_process_alive(Waiting) of
                true -> 
                    Waiting!{actions, [NewActions|Actions]},
                    accumulator_loop(Guardians, [], none, TimerRef, WebsocketPid);
                false ->
                    accumulator_loop(Guardians, [NewActions|Actions], none, TimerRef, WebsocketPid)
            end;

        {get_actions_blocking, Pid} when Actions == [] ->
            accumulator_loop(Guardians, [], Pid, TimerRef, WebsocketPid);
        ...
        {set_lease, LengthInMS} ->
            ...
            accumulator_loop(Guardians, Actions, Waiting, NewTimerRef, WebsocketPid);

        {websocket_pid, NewWebsocketPid} ->
            accumulator_loop(Guardians, Actions, Waiting, TimerRef, NewWebsocketPid);
    ...
    after ?TWENTY_SECONDS ->
        ...
        case TimerRef == undefined of
            ...
            false -> accumulator_loop(Guardians, Actions, Waiting, TimerRef, WebsocketPid)
        end
    end.
