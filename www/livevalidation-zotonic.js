...
// JG: 20121006: Adapted from Zotonic to Nitrogen
...
function addLiveValidation(element, args) {
	if (!$(element).data("LV_live_validation"))
	{
		$(element).data("LV_live_validation", new LiveValidation(element, args));
		$(element).validator = $(element).data("LV_live_validation");
//		$(element).data("LV_live_validation", new LiveValidation($(element).attr('id'), args));
	}
	return getLiveValidation(element);
}


function getLiveValidation(element) {
	return $(element).data("LV_live_validation");
}
...
LiveValidation.prototype = 
{
    ...
    asyncFieldClass: 'LV_async_validation',
     ...
     */
     ...
     createSpinnerSpan: function(){
         ...
         span.innerHTML = '<img src="/nitrogen/spinner.gif" height="16" width="16" alt="Validating..." />';
         return span;
     },
...
LiveValidationForm.getInstance = function(element){
  ...
  var instance = $(element).data("LV_live_validation_instance");
  if (!instance) {
      instance = new LiveValidationForm(element);
      $(element).data("LV_live_validation_instance", instance);
  }
  return instance;
}

LiveValidationForm.prototype = {
  validFormClass: 'LV_form_valid',
  invalidFormClass: 'LV_form_invalid',
  ...
  initialize: function(element){
    ...
    this.onInvalid = function() { 
		$(this).removeClass("LV_form_valid").addClass("LV_form_invalid"); 
		$(".LV_form_valid", this).hide();
		$(".LV_form_invalid", this).fadeIn();
	};
    this.onValid = function() { 
		$(this).removeClass("LV_form_invalid").addClass("LV_form_valid");
		$(".LV_form_invalid", this).hide();
		$(".LV_form_valid", this).fadeIn();
	};

    $(element).submit(function(event) {
      return (LiveValidation.massValidate(self.fields)) ? self.oldOnSubmit.call(this, event || window.event) !== false : false;
...
//                return LV_form_submit_validated_do(event);
//            }
//        } else {
...
//        }
  ...
  destroy: function(force){
	if (force || this.getFields().length == 0) {
	    ...
	    $(this.element).removeData("LV_live_validation_instance");
	    return true;
	} else {
		return false;
	}
  ...
  getFields: function() {
	...
	$("input,select,textarea", this.element).each(function() {
		var field = $(this).data('LV_live_validation');
		if (field) {
			fields.push(field);
		}
	});
...
//            Validate.fail(message);
//        } else if (paramsObj.LV_postback) {
//            // Perform the async postback
//            extraParams = new Array();
//            if (isSubmit) {
//                extraParams.push({name: 'LV_submitter', value: (submitTrigger && submitTrigger.name) ? submitTrigger.name : ''});
//            }
//            LV_queue_postback(paramsObj.LV_id, paramsObj.LV_postback, extraParams); 
//            return 'async';
//        } else {
//            return true;
//        }
